#pragma once

/**
 * \file training_sliding_data.h
 * \brief A file to be included only by "src/trainer_sliding.c".
 *		  Used to declare various strings and battle scripts used by the file.
 */

extern const TaskFunc sBattleIntroSlideFuncs[];

extern const u8 BattleScript_TrainerSlideMsgRet[];
extern const u8 BattleScript_TrainerSlideMsgEnd2[];

extern const u8 gText_DefaultTrainerDynamaxMsg[];
extern const u8 gText_TestTrainerDynamaxMsg[];
extern const u8 gText_RedDynamaxMsg[];

//For Unbound

extern const u8 sText_MirskleFirstMonDown[];
extern const u8 sText_VegaLastSwitchIn[];
extern const u8 sText_AliceLastSwitchIn[];
extern const u8 sText_MelLastSwitchIn[];
extern const u8 sText_GalavanLastSwitchIn[];
extern const u8 sText_GalavanLastLowHP[];
extern const u8 sText_BigMoLastSwitchIn[];
extern const u8 sText_TessyLastSwitchIn[];
extern const u8 sText_BenjaminLastSwitchIn[];
extern const u8 sText_MolemanLastLowHP[];
extern const u8 sText_EliasLastSwitchIn[];
extern const u8 sText_AnabelleLastSwitchIn[];
extern const u8 sText_PennyLastLowHP[];
extern const u8 sText_JaxLastLowHP[];
extern const u8 sText_Zeph1LowHP[];
extern const u8 sText_Zeph2LastSwitchIn[];
extern const u8 sText_Zeph3LastSwitchIn[];
extern const u8 sText_Aklove1LowHP[];
extern const u8 sText_Aklove2LowHP[];
extern const u8 sText_Aklove3LowHP[];
extern const u8 sText_MaximaLastSwitchIn[];
extern const u8 sText_RivalLeagueLastSwitchIn[];
extern const u8 sText_MarlonLeagueLastSwitchIn[];
extern const u8 sText_IvoryLeagueLastSwitchIn[];
extern const u8 sText_JaxPostGameLastLowHP[];
extern const u8 sText_MiloLeagueLastLowHP[];
extern const u8 sText_YoungsterLeagueLastSwitchIn[];
extern const u8 sText_RogueElectivire_DynamaxMsg[];




extern const u8 sTextBlueFirstMonDown[];
extern const u8 sTextBlueLastSwitchIn[];
extern const u8 sTextGiovanniFirstMonDown[];
extern const u8 sTextGiovanniLastSwitchIn[];
extern const u8 sTextBrockFirstMonDown[];
extern const u8 sTextBrockLastSwitchIn[];
extern const u8 sTextMistyFirstMonDown[];
extern const u8 sTextMistyLastSwitchIn[];
extern const u8 sTextLtSurgeFirstMonDown[];
extern const u8 sTextLtSurgeLastSwitchIn[];
extern const u8 sTextErikaFirstMonDown[];
extern const u8 sTextErikaLastSwitchIn[];
extern const u8 sTextKogaFirstMonDown[];
extern const u8 sTextKogaLastSwitchIn[];
extern const u8 sTextBlaineFirstMonDown[];
extern const u8 sTextBlaineLastSwitchIn[];
extern const u8 sTextSabrinaFirstMonDown[];
extern const u8 sTextSabrinaLastSwitchIn[];
extern const u8 sTextLoreleiFirstMonDown[];
extern const u8 sTextLoreleiLastSwitchIn[];
extern const u8 sTextBrunoFirstMonDown[];
extern const u8 sTextBrunoLastSwitchIn[];
extern const u8 sTextAgathaFirstMonDown[];
extern const u8 sTextAgathaLastSwitchIn[];
extern const u8 sTextLanceFirstMonDown[];
extern const u8 sTextLanceLastSwitchIn[];
extern const u8 sTextGreenFirstMonDown[];
extern const u8 sTextGreenLastSwitchIn[];

